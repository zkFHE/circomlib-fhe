...
The rounding operation takes care of the small added noise. However, to correctly recover the message the noise needs to be $|\mathrm{err}(a',b')| < q/(2t)$. The noise growth that occurs when performing computations with ciphertexts justify the need for refreshing their noise with a bootstrapping procedure.
...
The key switching procedure allows to change the key under which a LWE ciphertext is encrypted. This method is parametrized by a key switching modulus $Q_{ks}$ (in practice $Q_{ks} \approx 2^{14}$) and a key switching base $B_{ks}$ (in practice $B_{ks} \approx 128$). We can denote the number of digits of $Q_{ks}$ under base $B_{ks}$ as $dks = \lceil \log_{B_{ks}}Q_{ks} \rceil$.
...
that allows to multiply ciphertexts. We will now see how this operation looks like in detail.
...
Paralelly, given a RGSW ciphertext 
...
Finally, building upon the multiplication between a polynomial and a RLWE ciphertext,
...
Overall, we now know how to reduce that essential multiplication of ciphertexts to multiplications of polynomials in $\mathcal{R}_Q = \mathbb{Z}_Q[X]/(X^N+1)$. While we could just multiply polynomials via the *schoolbook* method and then reduce modulo $(X^N+1)$, the problem of multiplying polynomials has been deeply studied and there are far more efficient ways to do so. In particular, one can resort to the Number Theoretic Transform (NTT).
...
FOR i=0 to n-1
    ...
    (A'', B'') := (bin1 ⚬ prod1[0], bin1 ⚬ prod1[1])
    (A'', B'') := (A'', B'') + (bin2 ⚬ prod2[0], bin2 ⚬ prod2[1])
    (A'', B'') := (INTT(A''), INTT(B''))
...
Note that in TFHE only a signed decomposition and a subsequent NTT to the array is needed for the two RLWE-RGSW multiplications that take place.

Another interesting remark is that in practice, instead of a signed decomposition, an approximate signed decomposition that ignores the first digit of the decomposition is performed. This approximate version allows to save 2 NTTs per iteration (one per polynomial that is decomposed) at the cost of some tolerable additional error.
...
Now if we introduce the ciphertext $(a_s, b_s) \in \mathrm{LWE}_s^q(\tilde{m_s})$ and the function $\mathrm{f}$ into de accumulator, we obtain after the extraction a ciphertext 
...
FHEW and TFHE stand out for their fast function bootstrapping procedure that allows to reduce the noise of ciphertexts while evaluating a function. They can both be described in a unified framework following the approach of [MP21]. In such a framework, they both follow the structure of a first accumulator phase that performs the actual function evaluation and refreshing of the noise, followed by a modulus and key switching phase that transform the ciphertext into one under the original parameters. The only difference between both schemes under this framework is the way in which they perform the accumulator update or blind rotation. Overall, the functional bootstrapping presented can be leveraged to construct boolean gates such as the bootstrapped NAND gate from the last section.

The description contained in this blog post is a result of analyzing the theoretical description of the schemes, specially the ones found in [DM15] and [MP21], and reverse-engineering OpenFHE's implementation [OpenFHE]. We hope this post helps to bridge the gap between theory and practice that we consider existed for these schemes.


<!-- TODO: add glosary of parameters? -->
