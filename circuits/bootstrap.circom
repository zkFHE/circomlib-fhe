...
*/
template UpdateAcc(mode, n, N, q, Q, Bg, Br, bsk) {
    signal input acc_in[2][N];
    signal input a[n];
    signal output acc_out[2][N];

    if (mode == 0) {
        acc_out <== UpdateDM(n, N, q, Q, Br, Bg, bsk)(acc_in, a);
    }
}
...
*/
template BootstrapCore(mode, n, N, q, Q, Bg, Br, bsk, f) {
    signal input a_in[n], b_in;
    signal output a_out[N], b_out;

    var acc[2][N] = InitAcc(N, q, Q, f)(b_in);

    acc = UpdateAcc(mode, n, N, q, Q, Bg, Br, bsk)(acc, a_in);

    (a_out, b_out) <== ExtractFromAcc(N, Q)(acc);
}
...
/*
    ...
    same parameters) with less noise and encrypting f(m).
        ...
        - f: function to compute while bootstrapping. It maps Z_q to Z_Q and
             ...
             (Array of dimension q)
*/
template Bootstrap(mode, n, N, q, Q, Qks, Bks, Bg, Br, ksk, bsk, f) {
    signal input a_in[n], b_in;
    signal output a_out[n], b_out;

    signal (a_mid0[N], b_mid0) <== BootstrapCore(mode, n, N, q, Q, Bg, Br, bsk, f)(a_in, b_in);
